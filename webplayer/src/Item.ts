...
import { ZoomAndPanParams } from './ZoomAndPan';
...
export class Item {
  ...
  public effect_data?: string;
  /** Parsed from effect_data when effect_type === 'panzoom' */
  ...
  public constructor(json?: any) {
    ...
    if (this.effect_type === 'panzoom' && this.effect_data) {
      try {
        this.zoomAndPan = JSON.parse(this.effect_data);
      } catch (e) {
        console.warn('[Item] Failed to parse effect_data as ZoomAndPan:', this.effect_data);
      }
    }
    // Server sends title text as "texts" array (not "shapes") for title-only items
    // (artwork_id = NULL). Merge into shapes for unified rendering pipeline.
    // These text shapes are intentionally empty — Sequencer fills them client-side
    // from artwork title/author (temporary until server implements server-side fill).
