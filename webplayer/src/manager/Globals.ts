...
const detectPlaylistContentTypes = (playlist: Playlist | undefined) => {
  ...
  // Skip detection when Montages cache is empty — getMontage() would log spurious
  // "MONTAGE LOADING ERROR" errors for every montage. addMontage() will call this
  // again once the cache is populated.
  if (playlist && Object.keys(Montages).length === 0) {
...
export const setCurrentPlaylist = (p: Playlist | undefined) => {
  ...
  if (ThePlaylist === undefined && p === undefined) {
    ...
    // RACE CONDITION GUARD: On browser refresh the parent container fires
    // webPlayerNavigate immediately, which calls setCurrentPlaylist before the
    // WebSocket has had time to send 'montage' commands. Playlist.getMontage()
    // reads from the global Montages cache, so calling assumeNewPlaylist() now
    // would fail with "Global montages cache keys: []".
    // NOTE: detectPlaylistContentTypes is intentionally called AFTER this guard —
    // it calls getMontage() internally and would log spurious errors on empty cache.
    // Solution: defer both detectPlaylistContentTypes and assumeNewPlaylist() until
    // addMontage() populates the cache.
    if (p && Object.keys(Montages).length === 0) {
      LogHelper.log('setCurrentPlaylist', `Montages cache empty — deferring assumeNewPlaylist() until first montage arrives`);
      return; // addMontage() will call assumeNewPlaylist() once cache is ready
    }
    ...
    // The server (and parent app) can re-send the same playlist repeatedly;
    // without this check, assumeNewPlaylist() would restart playback on every message.
    LogHelper.log(
      'setCurrentPlaylist',
    ...
    // Same playlist ID — check if montage order changed (e.g. user reordered montages in CMS).
    // The signature is a comma-joined list of montage IDs in order, generated by Playlist constructor.
    // A signature change means the sequencer must reinitialise with the new order, even though
    // the playlist ID hasn't changed.
    const oldSig = ThePlaylist?.montageOrderSignature;
    const newSig = p.montageOrderSignature;
    if (oldSig && newSig && oldSig !== newSig) {
      LogHelper.log(
        'setCurrentPlaylist',
        `Playlist ${p.id}: montage order changed (${oldSig} → ${newSig}), reinitialising sequencer`
      );
      ThePlaylist = p;
      detectPlaylistContentTypes(p);
      Sequencer.assumeNewPlaylist(p);
