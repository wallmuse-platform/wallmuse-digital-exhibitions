...
export function ConfigureDisplays({ t, onClose, handleAction }) {
  ...
  const environmentGroup = useMemo(() => {
    ...
    if (multipleCheck.hasMultiple) {
      console.warn(
        "[Configure] Multiple active environments detected:",
        multipleCheck,
      );
    }
  ...
  const handleEnvironmentGroupUpdate = (updatedEnvironmentGroup) => {
    console.log(
      "[Configure] Updating environment group:",
      updatedEnvironmentGroup,
    );

    // Convert back to environments array and update state
    const updatedEnvironments = updatedEnvironmentGroup.map(
      ({ environment, screens }) => ({
        ...environment,
        screens: screens,
      }),
  ...
  const handleRemoveScreen = (id) => {
    const screenToRemove = environments
      .flatMap((env) => env.screens || [])
      .find((screen) => String(screen.id) === String(id));
    ...
    const environmentOfScreen = environments.find((env) =>
      env.screens?.includes(screenToRemove),
    );

    if (
      environmentOfScreen?.name !== "Web player" &&
      environmentOfScreen?.screens?.length === 1
    ) {
        ...
        const newEnvironments = environments.map((env) => {
          if (env.id === environmentOfScreen?.id) {
            return {
              ...
              screens:
                env.screens?.filter(
                  (screen) => String(screen.id) !== String(id),
                ) || [],
            };
          }
          return env;
  ...
  const handleRemoveEnvironment = (id) => {
    ...
    const environmentToRemove = environments.find(
      (env) => String(env.id) === String(id),
    );
    ...
    if (isEnvironmentActive(environmentToRemove)) {
      console.log(
        "[Configure] Cannot remove active environment:",
        environmentToRemove,
      );
    ...
    console.log(
      "[Configure] Environment is not active, proceeding with removal",
    );
  ...
  const handleDeactivateEnvironment = async (id) => {
    ...
    const environmentToDeactivate = environments.find(
      (env) => String(env.id) === String(id),
    );
    ...
    if (!isEnvironmentActive(environmentToDeactivate)) {
      console.log(
        "[Configure] Environment is already inactive:",
        environmentToDeactivate,
      );
      return;
    }
    ...
    try {
      ...
      if (
        environmentToDeactivate.screens &&
        environmentToDeactivate.screens.length > 0
      ) {
        console.log(
          `[Configure] Deactivating ${environmentToDeactivate.screens.length} screens first`,
        );

        for (const screen of environmentToDeactivate.screens) {
          if (isScreenActive(screen)) {
            console.log(
              `[Configure] Deactivating screen ${screen.id} (${screen.name})`,
            );
            try {
              await deactivateScreen(screen.id);
              console.log(
                `[Configure] Screen ${screen.id} deactivated successfully`,
              );
            } catch (screenError) {
              console.error(
                `[Configure] Error deactivating screen ${screen.id}:`,
                screenError,
              );
            ...
            console.log(
              `[Configure] Screen ${screen.id} is already inactive, skipping`,
            );
          }
        }
      }
