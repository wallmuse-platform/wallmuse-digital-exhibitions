...
export function ConfigureEnvironmentScreen({
  ...
  console.log(
    "ConfigureEnvironmentScreen: environmentGroup:",
    environmentGroup,
  );
  ...
  useEffect(() => {
    if (environmentGroup && environmentGroup.length > 0) {
      ...
      if (results.issues.length > 0) {
        console.log(
          "[ConfigureEnvironmentScreen] Validation issues found:",
          results,
        );
      }
    }
  ...
  const filteredEnvironmentGroup = environmentGroup
    .map(({ environment, screens }) => {
      let filteredScreens;
    ...
    .filter(({ environment, screens }) => {
      if (showOnlyActive) {
        ...
        return (
          isEnvironmentActive(environment) ||
  ...
  const handleScreenToggle = async (screen) => {
    ...
    console.log(
      "[Configure] About to call handleAction, userId:",
      getUserId(),
      "isDemo:",
      isDemoAccount(getUserId()),
    );
    ...
    handleAction(
      async () => {
        console.log(
          "[Configure] Inside handleAction callback - executing screen toggle",
        );
        const isCurrentlyActive = isScreenActive(screen);
        console.log(
          `[ConfigureEnvironmentScreen] Toggling screen ${screen.id} from ${isCurrentlyActive ? "on" : "off"} to ${isCurrentlyActive ? "off" : "on"}`,
        );
        ...
        try {
          ...
          const updatedEnvironmentGroup = environmentGroup.map(
            ({ environment, screens }) => ({
              environment,
          ...
          console.error(
            `[ConfigureEnvironmentScreen] Error toggling screen ${screen.id}:`,
            error,
          );
          alert(
            `Error ${isCurrentlyActive ? "deactivating" : "activating"} screen. Please try again.`,
          );
        }
      },
  ...
  const handleEnvironmentToggle = async (environment) => {
    ...
    handleAction(
      async () => {
        ...
        console.log(
          `[ConfigureEnvironmentScreen] Toggling environment ${environment.id} from ${isCurrentlyActive ? "alive" : "dead"} to ${isCurrentlyActive ? "dead" : "alive"}`,
        );
        ...
        try {
          ...
          console.error(
            `[ConfigureEnvironmentScreen] Error toggling environment ${environment.id}:`,
            error,
          );
          alert(
            `Error ${isCurrentlyActive ? "deactivating" : "activating"} environment. Please try again.`,
          );
        }
      },
      false, // Not premium content
  ...
  const getScreenStatusTooltip = (screen) => {
    ...
    if (!hasValidDimensions(screen))
      return "Screen active but may need page refresh for dimensions";
  ...
  return (
    <>
            ...
            <Switch
              ...
              onChange={(e) => {
                ...
                handleAction(
                  () => {
                    console.log(
                      "[Configure] Inside handleAction callback - executing setShowOnlyActive",
                    );
      ...
      {filteredEnvironmentGroup &&
        filteredEnvironmentGroup.map(({ environment, screens }, index) => {
              ...
              >
                ...
                <div
                  style={{
                    ...
                    overflow: "hidden",
                    visibility:
                      hoveredItem === screen.id ? "visible" : "hidden",
                  }}
                >
                  <Tooltip
                    title={
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      overflow: "hidden",
                      visibility:
                        hoveredItem === environment.id ? "visible" : "hidden",
                    }}
                  >
                    <IconButton
                      onClick={() => {
