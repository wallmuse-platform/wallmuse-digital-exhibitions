...
export const SessionProvider = ({ children }) => {
  ...
  console.log(
    "[SessionContext] rootElement?.dataset?.wpLoggedIn",
  ...
  const updateHouseInDOM = (houseId) => {
    ...
    if (rootElement) {
      ...
      console.log(
        "[SessionContext] DOM updated. New dataset:",
        rootElement.dataset,
      );
    }
    ...
    const existingWmHouse = JSON.parse(
      localStorage.getItem("wm-house") || "{}",
    );
    ...
    console.log(
      "[SessionContext] Updated wm-house localStorage:",
      updatedWmHouse,
    );
  };
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      console.log(
        "[SessionContext] Starting session initialization with userId:",
        userId,
      );
      ...
      if (!wpLoggedIn && !isGuest) {
        console.log(
          "[SessionContext] WordPress reports user not logged in and not a guest, skipping initialization",
        );
        if (isMounted) {
          resetSession();
        }
      ...
      try {
        ...
        if (userDetails?.houses?.length > 0) {
          console.log(
            "[SessionContext] User has existing houses:",
            userDetails.houses,
          );
          ...
          if (existingHouse && existingHouse.autostart_playlist !== undefined) {
            const autostartValue =
              typeof existingHouse.autostart_playlist === "boolean"
                ...
                : existingHouse.autostart_playlist === "1" ||
                  existingHouse.autostart_playlist === 1;
          ...
          try {
            ...
            if (result.success) {
              console.log(
                "[SessionContext] House creation successful:",
                result,
              );
              ...
              if (cleanedCount > 0) {
                console.log(
                  `[SessionContext] Cleaned ${cleanedCount} erroneous house fingerprints during account setup`,
                );
              }
              ...
              const autostartResult = await setHouseAutostart(
                result.houseId,
                1,
              );
              console.log(
                "[SessionContext] Autostart setting result:",
                autostartResult,
              );
        ...
        console.error(
          "[SessionContext] Error getting user profile:",
          profileError,
        );
        if (isMounted) {
          resetSession();
    ...
    if (userId && (wpLoggedIn || isGuest)) {
      ...
      console.warn(
        "[SessionContext] No userId or (not logged in to WordPress and not a guest), resetting session",
      );
      if (isMounted) {
        resetSession();
      }
