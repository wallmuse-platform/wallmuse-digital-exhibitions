...
import {
  requestScreenPermissionForEnvironment,
  getScreenDimensions,
} from "../utils/requestScreenPermission";
...
export const EnvironmentsProvider = ({ children }) => {
  ...
  const [backendCurrentPlaylist, setBackendCurrentPlaylist] =
    useState(undefined);
  ...
  const waitForScreenDimensions = async (screenId, environmentId) => {
    console.log(
      `[waitForScreenDimensions] Waiting for screen ${screenId} to get dimensions...`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      const interval = setInterval(async () => {
        ...
        try {
          ...
          if (!screen) {
            console.log(
              `[waitForScreenDimensions] Check ${attempts}/${maxAttempts}: Screen ${screenId} not found`,
            );
          ...
          console.log(
            `[waitForScreenDimensions] Check ${attempts}/${maxAttempts}: Screen ${screenId} has width=${screen.width}, height=${screen.height}, on=${screen.on}`,
          );
          ...
          if (
            ...
            console.log(
              `[waitForScreenDimensions] Screen dimensions populated successfully!`,
            );
            clearInterval(interval);
          ...
          if (attempts === 3) {
            try {
              console.log(
                `[waitForScreenDimensions] Still no dimensions after ${attempts} attempts, activating again...`,
              );
              const dimensions = {
              ...
              console.error(
                `[waitForScreenDimensions] Error reactivating screen:`,
                error,
              );
            }
          }
          ...
          if (attempts >= maxAttempts) {
            console.log(
              `[waitForScreenDimensions] Max attempts reached, giving up`,
            );
            clearInterval(interval);
          ...
          console.error(
            `[waitForScreenDimensions] Error checking screen dimensions:`,
            error,
          );
          clearInterval(interval);
          reject(error);
  ...
  const fetchEnvironmentDetails = useCallback(async (forceRefresh = false) => {
    console.log(
      `[fetchEnvironmentDetails] Fetching user details... (forceRefresh: ${forceRefresh ? "true" : "false"})`,
    );
    ...
    try {
      ...
      if (housesData.length > 0) {
        ...
        console.log(
          "[fetchEnvironmentDetails] Raw housesData[0]:",
          housesData[0],
        );
        console.log(
          "[fetchEnvironmentDetails] housesData[0]?.environments:",
          housesData[0]?.environments,
        );
        ...
        console.log(
          "[fetchEnvironmentDetails] Found environments:",
          houseEnvironments,
        );
        console.log(
          "[fetchEnvironmentDetails] houseEnvironments.length:",
          houseEnvironments.length,
        );
        ...
        const accountCreationProcess = {
          ...
          environments: houseEnvironments.map((env) => ({
            id: env.id,
            ip: env.ip || "none",
            screenDimensions: env.screens?.[0]
              ? `${env.screens[0].width}x${env.screens[0].height}`
              : "no_screen",
          })),
          flags: {
        ...
        if (houseId) {
          console.log(
            "[EnvironmentsContext] Updating DOM with discovered house ID:",
            houseId,
          );
        ...
        if (isNewAccount && newAccountHouseId === houseId) {
          console.log(
            "[fetchEnvironmentDetails] Processing new account setup for house:",
            houseId,
          );

          try {
            console.log(
              "[fetchEnvironmentDetails] Copying playlists from guest account...",
            );
            ...
            if (!copiedHouses.includes(houseId)) {
              ...
              console.log(
                "[fetchEnvironmentDetails] Playlists already copied for house:",
                houseId,
              );
            }
            ...
            if (houseEnvironments.length > 0) {
              ...
              console.log(
                "[fetchEnvironmentDetails] Examining environments and screens:",
              );
              houseEnvironments.forEach((env) => {
                ...
                if (env.screens && env.screens.length > 0) {
                  env.screens.forEach((screen) => {
                    console.log(
                      `  Screen ${screen.id}: enabled=${screen.enabled}, on=${screen.on}, width=${screen.width}, height=${screen.height}`,
                    );
                  });
                } else {
              ...
              const masterEnv = houseEnvironments.find(
                (env) => env.ip === "127.0.0.1",
              );
              const cryptKeyEnvs = houseEnvironments.filter(
                (env) => env.crypt_key && !env.ip,
              );

              if (masterEnv) {
                console.log(
                  "[fetchEnvironmentDetails] Found master environment with IP 127.0.0.1:",
                  masterEnv.id,
                );
                ...
                if (cryptKeyEnvs.length > 0) {
                  console.log(
                    "[fetchEnvironmentDetails] Found non-master environments with crypt_key:",
                  ...
                  for (const env of cryptKeyEnvs) {
                    try {
                      ...
                      console.log(
                        `[fetchEnvironmentDetails] Removed non-master environment ${env.id}`,
                      );
                    } catch (error) {
                      console.error(
                        `[fetchEnvironmentDetails] Error removing environment ${env.id}:`,
                        error,
                      );
                    }
                  }
                  ...
                  if (refreshResponse?.data?.houses?.[0]?.environments) {
                    setEnvironments(
                      refreshResponse.data.houses[0].environments,
                    );
                  }
                }
              }
              ...
              console.log(
                "[fetchEnvironmentDetails] hasActiveScreen",
                hasActiveScreen,
              );

              if (hasActiveScreen) {
                // Find environments with faulty screens that are named "Web player"
                const environmentsWithFaultyScreens = houseEnvironments.filter(
                  (env) =>
                    env.name === "Web player" &&
                    env.screens?.some(
                      (screen) =>
                ...
                console.log(
                  "[fetchEnvironmentDetails] Environments with faulty screens:",
                  environmentsWithFaultyScreens,
                );
                ...
                for (const env of environmentsWithFaultyScreens) {
                  ...
                  const isEssential =
                    houseEnvironments.length === 1 || env.ip === "127.0.0.1";

                  console.log(
                    `[fetchEnvironmentDetails] Environment ${env.id} is ${isEssential ? "essential" : "non-essential"}`,
                  );

                  if (isEssential) {
                    console.log(
                      `[fetchEnvironmentDetails] Fixing screens for essential environment ${env.id}`,
                    );

                    // Find the faulty screen
                    const faultyScreen = env.screens.find(
                      (screen) =>
                        screen.enabled === "1" &&
                        (screen.width === "0" ||
                          screen.height === "0" ||
                    ...
                    if (faultyScreen) {
                      try {
                        console.log(
                          `[fetchEnvironmentDetails] Activating faulty screen ${faultyScreen.id}`,
                        );
                        ...
                        console.log(
                          `[fetchEnvironmentDetails] Activating screen ${faultyScreen.id} with:`,
                          {
                        ...
                        console.log(
                          `[fetchEnvironmentDetails] Screen ${faultyScreen.id} activated with dimensions`,
                        );
                        ...
                        const waitResult = await waitForScreenDimensions(
                          faultyScreen.id,
                          env.id,
                        );
                        ...
                        if (waitResult.success) {
                          console.log(
                            `[fetchEnvironmentDetails] Screen dimensions successfully populated!`,
                          );
                          // Update environments with the updated screen
                          const updatedResponse = await detailsUser(Date.now());
                          if (
                            updatedResponse?.data?.houses?.[0]?.environments
                          ) {
                            setEnvironments(
                              updatedResponse.data.houses[0].environments,
                            );
                          ...
                          const refreshAlreadyShown =
                            localStorage.getItem("refreshShown") === "true";
                          ...
                          if (refreshAttempts >= 2) {
                            console.log(
                              `[fetchEnvironmentDetails] Multiple refresh attempts failed. Giving up.`,
                            );
                          ...
                          // If we've already shown one refresh, try second refresh
                            ...
                            console.log(
                              `[fetchEnvironmentDetails] First refresh didn't work, trying silent second refresh`,
                            );
                            // Remove needsRefresh to avoid both mechanisms running
                            localStorage.removeItem("needsRefresh");
                            console.log(
                              `[fetchEnvironmentDetails l.356] setItem('needsSecondRefresh)`,
                            );
                            localStorage.setItem("needsSecondRefresh", "true");
                          }
                          // First detection of faulty screen, use normal refresh
                          else if (
                            localStorage.getItem("needsRefresh") !== "true"
                          ) {
                            console.log(
                              `[fetchEnvironmentDetails l.361] Setting first refresh flag`,
                            );
                        ...
                        console.error(
                          `[fetchEnvironmentDetails] Error activating screen ${faultyScreen.id}:`,
                          error,
                        );
                      }
                    }
                ...
                if (screenFixed) {
                  console.log(
                    "[fetchEnvironmentDetails] Successfully fixed essential environment, now removing non-essential ones",
                  );
                  ...
                  for (const env of environmentsWithFaultyScreens) {
                    ...
                    if (!isEssential) {
                      try {
                        console.log(
                          `[fetchEnvironmentDetails] Removing non-essential environment ${env.id} with faulty screens`,
                        );
                        await removeEnvironment(env.id);
                        console.log(
                          `[fetchEnvironmentDetails] Successfully removed environment ${env.id}`,
                        );
                      } catch (error) {
                        console.error(
                          `[fetchEnvironmentDetails] Error removing environment ${env.id}:`,
                ...
                console.log(
                  "[fetchEnvironmentDetails] No active screens found, will create one",
                );
                ...
                if (
                  masterEnv &&
                  (!masterEnv.screens || masterEnv.screens.length === 0)
                ) {
                  console.log(
                    "[fetchEnvironmentDetails] Creating screen for master environment",
                  );
                  try {
                    ...
                    if (
                      screenRefreshResponse?.data?.houses?.[0]?.environments
                    ) {
                      const refreshedEnv =
                        screenRefreshResponse.data.houses[0].environments.find(
                          (e) => e.id === masterEnv.id,
                        );

                      if (
                        refreshedEnv &&
                        refreshedEnv.screens &&
                        refreshedEnv.screens.length > 0
                      ) {
                        ...
                        if (screenId) {
                          try {
                            ...
                            if (waitResult.success) {
                              console.log(
                                `[fetchEnvironmentDetails] New screen dimensions successfully populated!`,
                              );
                              // Update environments with the updated screen
                              const updatedResponse = await detailsUser(
                                Date.now(),
                              );
                              if (
                                updatedResponse?.data?.houses?.[0]?.environments
                              ...
                              if (isActivationComplete && isRefreshNeeded) {
                                console.log(
                                  `[fetchEnvironmentDetails l.456] Account activation complete but screen needs second refresh`,
                                );
                                localStorage.setItem(
                                  "needsSecondRefresh",
                                  "true",
                              ...
                              if (
                                ...
                                console.log(
                                  `[fetchEnvironmentDetails l.463] setItem('needsRefresh')`,
                                );
                                localStorage.setItem("needsRefresh", "true");
                              }
                            }
                    ...
                    console.log(
                      "[fetchEnvironmentDetails] Final refresh after setup",
                    );
                    ...
                    console.error(
                      "[fetchEnvironmentDetails] Error creating screen for master environment:",
                      error,
                    );
                  }
                }
            ...
            console.error(
              "[fetchEnvironmentDetails] Error copying playlists:",
              playlistError,
            );
            localStorage.removeItem("accountJustCreated");
            localStorage.removeItem("newAccountHouseId");
        ...
        const backendCurrentPlaylist =
          housesData[0]?.current_playlist || undefined;
        ...
        if (
          ...
          getPlaylistById(backendCurrentPlaylist)
            ...
            .catch((error) => {
              console.error(
                "[EnvironmentsContext] Error fetching initial playlist:",
                error,
              );
            });
        }
  ...
  useEffect(() => {
    ...
    if (isNewAccount) {
      console.log(
        "[EnvironmentsContext] Checking for faulty screens in",
        environments.length,
        "environments",
      );
      ...
      if (hasFaultyScreen) {
        console.log(
          "[EnvironmentsContext] Found faulty screens, setting refresh needed",
        );
  ...
  const handlePlaylistChange = useCallback(
    async (newPlaylistId, playlistObject = null, position = null) => {
      console.log(
        "[handlePlaylistChange] New playlist selected:",
        newPlaylistId,
        "position:",
        position,
        "hasPlaylistObject:",
      ...
      console.log(
        "[handlePlaylistChange] Updating currentPlaylist immediately from",
        currentPlaylist,
        "to",
        newPlaylistId,
      );
      ...
      console.log(
        "[handlePlaylistChange] Set window.currentPlaylistForNav:",
  ...
  useEffect(() => {
    ...
    const handleHouseCreated = async (event) => {
      ...
      setTimeout(async () => {
        try {
          console.log(
            "[EnvironmentsContext] Re-fetching environment details after house creation",
          );
          await fetchEnvironmentDetails(true);
          console.log(
            "[EnvironmentsContext] Environment refresh complete after house creation",
          );
        } catch (error) {
          console.error(
            "[EnvironmentsContext] Error handling house creation:",
            error,
          );
        }
      }, 1000);
    ...
    const handleChildPlaylistChange = (event) => {
      console.log(
        "[EnvironmentsContext] Child playlist changed event:",
        event.detail,
      );
      const { playlistId, playlist } = event.detail;
      if (playlistId && String(playlistId) !== String(currentPlaylist)) {
        console.log(
          `[EnvironmentsContext] Updating parent playlist state from ${currentPlaylist} to ${playlistId}`,
        );
        ...
        if (playlist) {
          ...
          console.log(
            `[EnvironmentsContext] ✅ INSTANT: Stored full playlist data in window.currentPlaylist: ${playlist.name || playlist.id}`,
          );
        } else {
          console.log(
            `[EnvironmentsContext] ⚠️ No playlist object in event, child will need to retry or use WebSocket`,
          );
        }
    ...
    window.addEventListener(
      "child-playlist-changed",
      handleChildPlaylistChange,
    );
  ...
  useEffect(() => {
    const needsSecondRefresh =
      localStorage.getItem("needsSecondRefresh") === "true";

    if (needsSecondRefresh) {
      console.log(
        "[EnvironmentsContext] needsSecondRefresh detected - refreshing environment data",
      );
      ...
      const handleDataRefresh = async () => {
        try {
          ...
          console.log(
            "[EnvironmentsContext] Fetching fresh environment data...",
          );
          ...
          localStorage.removeItem("needsSecondRefresh");
          console.log(
            "[EnvironmentsContext] Data refresh completed, needsSecondRefresh cleared",
          );
          ...
          console.error(
            "[EnvironmentsContext] Error during data refresh:",
            error,
          );
