...
const GuestActionPopup = ({
  ...
  console.log(
    "[GuestActionPopup] Rendering popup. isPremiumContent:",
    isPremiumContent,
    "currentTheme:",
    currentTheme,
  );
  ...
  const handleGuestAccount = async () => {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("account-phase-change", {
      ...
      if (onContinueWithTemp && typeof onContinueWithTemp === "function") {
        console.log(
          "[GuestActionPopup] Calling onContinueWithTemp with result",
        );
      ...
      alert(
        t("temp_account_error") ||
          "Error creating temporary account. Please try again.",
      );
    } finally {
  ...
  const handleClose = () => {
    ...
    if (onClose && typeof onClose === "function") {
      ...
      console.error(
        "[GuestActionPopup] onClose is not a function or not provided",
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showPremiumMessage && (
          <p style={{ marginBottom: "20px", fontSize: fontSize.message }}>
            {t("premium_content_message") ||
              "This content requires a premium subscription. Subscribe now to unlock all features."}
          </p>
        )}
        ...
        {!showPremiumMessage && (
          <p style={{ marginBottom: "20px", fontSize: fontSize.message }}>
            {t("account_options_message") ||
              "You need an account to perform this action. Choose an option:"}
          </p>
        )}
