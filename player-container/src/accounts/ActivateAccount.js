  ...
  const handleActivate = () => {
    ...
    if (ageResponse !== "na" || operaResponse !== "na") {
      ...
      localStorage.setItem(
        `${window.location.hostname}_survey_submitted`,
        "true",
      );
    }
    ...
    if (isCloneAccount) {
      try {
        ...
        console.log(
          "[ActivateAccount] Initialized empty data structures for new account",
        );
      } catch (e) {
        console.error(
          "[ActivateAccount] Error initializing data structures:",
          e,
        );
      }
    }
    ...
    if (!isCloneAccount) {
      ...
      console.log(
        "[ActivateAccount l.90] Using needsSecondRefresh for existing account",
      );
