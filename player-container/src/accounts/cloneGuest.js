...
export const cloneGuest = async (updateSession = null) => {
  ...
  try {
    ...
    if (!newUser?.id) {
      throw new Error(
        "[cloneGuest] Failed to create a guest user. No ID returned.",
      );
    }
 ...
 */
export const handleGuestAction = async (
  action,
  updateSession = null,
  options = {},
) => {
  ...
  if (options.showConfirm) {
    ...
    if (!window.confirm(confirmMsg)) {
      console.log(
        "[GuestActionHandler] User canceled temporary account creation",
      );
      return null;
    }
  }
  ...
  try {
    ...
    console.log(
      "[GuestActionHandler] Temporary account created:",
      cloneResult.userId,
    );
