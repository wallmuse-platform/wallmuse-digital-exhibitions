...
const useGuestActionPopup = () => {
  ...
  useEffect(() => {
    const refreshUserStatus = () => {
      ...
      console.log(
        "[useGuestActionPopup] Refreshing user status, old:",
        localUserId,
        "new:",
        currentId,
      );
  ...
  useEffect(() => {
    ...
    if (shouldCreateGuest) {
      ...
      if (isDemo) {
        console.log(
          "[useGuestActionPopup] create_guest parameter detected from external app, auto-showing popup",
        );
        setShowPopup(true);
        ...
        const cleanUrl =
          ...
          (params.get("lang") ? `?lang=${params.get("lang")}` : "");
  ...
  const handleAction = (action, isPremium = false) => {
    ...
    if (!isDemo) {
      console.log(
        "[useGuestActionPopup] Not a guest, executing action directly",
      );
      action();
      return;
    }
  ...
  const continueWithTemp = (result) => {
    console.log(
      "[useGuestActionPopup] continueWithTemp called, result:",
      result,
    );
    ...
    console.log(
      "[useGuestActionPopup] Updated local user ID to:",
      result.userId,
    );
    ...
    if (rootElement && rootElement.dataset) {
      console.log("[useGuestActionPopup] Current DOM dataset before update:", {
        ...rootElement.dataset,
      });
      ...
      console.log("[useGuestActionPopup] Updated DOM dataset:", {
        ...rootElement.dataset,
      });
    } else {
      console.error(
        "[useGuestActionPopup] Could not find root element or dataset",
      );
    }

    // Dispatch event to notify all components
    window.dispatchEvent(
      new CustomEvent("user-status-changed", {
