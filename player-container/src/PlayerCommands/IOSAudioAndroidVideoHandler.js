...
const IOSAudioAndroidVideoHandler = ({
  ...
  useEffect(() => {
    if (!isMobile) {
      console.log(
        "[IOSAudioAndroidVideoHandler] Not a mobile device, skipping interceptor",
      );
      return;
    }

    console.log(
      "[IOSAudioAndroidVideoHandler] Installing video.play() interceptor for mobile",
    );
      ...
      return playPromise.catch((error) => {
        if (error.name === "NotAllowedError") {
          ...
          if (!window._blockedVideoElements.includes(this)) {
            ...
            console.log(
              "[IOSAudioAndroidVideoHandler] Stored blocked video element, total:",
              window._blockedVideoElements.length,
            );
          }
        }
    ...
    console.log(
      "[IOSAudioAndroidVideoHandler] ✅ Video play interceptor installed",
    );
    ...
    return () => {
      ...
      console.log(
        "[IOSAudioAndroidVideoHandler] Video play interceptor removed",
      );
    };
  }, [isMobile]);
  ...
  useEffect(() => {
    if (!isIOS) {
      console.log(
        "[IOSAudioAndroidVideoHandler] Not iOS, skipping AudioContext check",
      );
      return;
    }

    console.log(
      "[IOSAudioAndroidVideoHandler] iOS device detected, checking audio context...",
    );
    ...
    const checkAudioContext = () => {
      ...
      if (window.AudioContext || window.webkitAudioContext) {
        ...
        console.log(
          "[IOSAudioAndroidVideoHandler] AudioContext available:",
          !!AudioContextClass,
        );

        if (!window.globalAudioContext) {
          console.log(
            "[IOSAudioAndroidVideoHandler] Creating new audio context",
          );
        ...
        console.log(
          "[IOSAudioAndroidVideoHandler] Audio context state:",
          audioContext.state,
        );

        if (audioContext.state === "suspended") {
          console.log(
            "[IOSAudioAndroidVideoHandler] Audio context suspended - showing prompt",
          );
          setBlockReason("audio");
          setShowPrompt(true);
        } else {
          console.log(
            "[IOSAudioAndroidVideoHandler] Audio context running - no prompt needed",
          );
        }
      } else {
  ...
  const handleEnableMedia = useCallback(async () => {
    console.log(
      "[IOSAudioAndroidVideoHandler] handleEnableMedia clicked, reason:",
      blockReason,
    );
    ...
    try {
      ...
      if (isIOS && (window.AudioContext || window.webkitAudioContext)) {
        ...
        const audioContext = window.globalAudioContext;
        console.log(
          "[IOSAudioAndroidVideoHandler] Audio context state before resume:",
          audioContext.state,
        );
        ...
        if (audioContext.state === "suspended") {
          ...
          console.log(
            "[IOSAudioAndroidVideoHandler] ✅ Audio context resumed, new state:",
            audioContext.state,
          );
        }
      }
      ...
      if (
        ...
        window._blockedVideoElements.length > 0
      ) {
        console.log(
          "[IOSAudioAndroidVideoHandler] Retrying",
          window._blockedVideoElements.length,
          "blocked video(s)",
        );
        ...
        for (const video of window._blockedVideoElements) {
          try {
            ...
            console.warn(
              "[IOSAudioAndroidVideoHandler] ⚠️ Video play retry failed:",
              retryError.name,
            );
          }
        }
      ...
      if (
        currentMutedValue === null ||
        JSON.parse(currentMutedValue) === true
      ) {
        console.log(
          "[IOSAudioAndroidVideoHandler] Auto-unmuting (was:",
          currentMutedValue,
          ")",
        );
        ...
        window.dispatchEvent(
          new CustomEvent("wallmuse-unmute", { detail: { muted: false } }),
        );
        console.log(
          "[IOSAudioAndroidVideoHandler] ✅ Auto-unmute applied and event dispatched",
        );
      }
  ...
  console.log(
    "[IOSAudioAndroidVideoHandler] Render decision - showPrompt:",
    showPrompt,
  );

  if (!showPrompt) {
    console.log(
      "[IOSAudioAndroidVideoHandler] Not rendering - showPrompt is false",
    );
    return null;
  }

  console.log(
    "[IOSAudioAndroidVideoHandler] Rendering prompt with variant:",
    variant,
    "reason:",
    blockReason,
  );
  ...
  const getMessage = () => {
    if (blockReason === "video")
      return t("tapToEnableVideo", "Tap to enable video playback");
    if (blockReason === "audio")
      return t("tapToEnableAudio", "Tap to enable audio playback");
