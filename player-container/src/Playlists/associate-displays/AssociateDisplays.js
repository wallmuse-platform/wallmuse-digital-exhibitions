...
const AssociateDisplays = ({
  ...
  // so use playlists directly — not a filtered copy.
  // If playlistIndex is undefined or the indexed playlist doesn't contain the montage,
  // fall back to searching all playlists by montageId.
  const allPlaylists = Array.isArray(playlists) ? playlists : [];
  const indexedPlaylist = playlistIndex !== undefined ? allPlaylists[playlistIndex] : undefined;
  const selectedPlaylist =
    indexedPlaylist?.montages?.some((m) => String(m.id) === String(montageId))
      ? indexedPlaylist
      : allPlaylists.find((p) =>
          p.montages?.some((m) => String(m.id) === String(montageId)),
        );
  const relatedMontage = selectedPlaylist?.montages?.find(
    (m) => String(m.id) === String(montageId),
  );

  console.log("[AssociateDisplays] lookup:", {
    montageId,
    playlistIndex,
    playlistsTotal: allPlaylists.length,
    selectedPlaylistId: selectedPlaylist?.id,
    selectedPlaylistName: selectedPlaylist?.name,
    montageIdsInPlaylist: selectedPlaylist?.montages?.map((m) => m.id),
    relatedMontageFound: !!relatedMontage,
  });
  ...
  const filteredEnvironments = React.useMemo(() => {
    ...
    return environments.filter((env) => {
      ...
      const hasActiveScreens = env.screens.some(
        (screen) =>
  ...
  const handleTrackChange = async (montageId, screenId, trackNumber) => {
    ...
    if (!Array.isArray(environments) || !Array.isArray(playlists)) {
      console.error("[AssociateDisplays] Missing required data:", {
        environments,
        playlists,
      });
      return;
    ...
    let screenIndex =
      montage.screens?.findIndex((s) => s.id === screenId) ?? -1;
    const currentTrack =
      screenIndex !== -1 ? montage.screens[screenIndex]?.seq : "";
    ...
    if (!isTrackChanging || !handleAction) {
      console.log(
        "[AssociateDisplays] No track change or no handleAction, proceeding directly",
      );
      return;
    }
    ...
    handleAction(
      async () => {
        console.log(
          "[AssociateDisplays] Executing track change after guest check",
        );
        ...
        if (screenIndex === -1) {
          ...
          const environment = environments?.find((e) =>
            e?.screens?.some((s) => s.id === screenId),
          );
          const newScreen = environment?.screens?.find(
            (s) => s.id === screenId,
          );
          if (!newScreen) {
            console.error(
              "[AssociateDisplays HandleTrackChange] Screen not found in environments!",
              { screenId },
            );
            return;
          }
        ...
        try {
          ...
          console.log(
            "[AssociateDisplays] Server call montageId, house, seqNumberInt, screenId, session",
            montageId,
            house,
            seqNumberInt,
            screenId,
          ...
          const response = await setTrackScreen(
            montageId,
            seqNumberInt,
            screenId,
            session,
  ...
  const renderScreen = (screen, env) => {
    ...
    const screenDetail = relatedMontage?.screens?.find(
      (s) => String(s.id) === String(screen.id),
    );
    ...
    return (
      <Paper
        ...
        sx={{
          ...
          padding: 2,
          borderLeft:
            screen.on === "1" ? `4px solid #4CAF50` : `4px solid #9e9e9e`,
          borderRadius: 1,
        }}
      >
        <Box
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
            ...
            >
              {isMobile && (
                <SmartphoneIcon sx={{ fontSize: iconSize, color: iconColor }} />
              )}
              {isTablet && (
                <TabletAndroidIcon
              ...
              {(isHD || isUHD || isSmartTV) && (
                <TvIcon sx={{ fontSize: iconSize, color: iconColor }} />
              )}
            </Badge>
          ...
          >
            ...
            <Typography>
              {t("Dimensions")}: {screen.width} x {screen.height}
            </Typography>
            <Typography>
              {screen.enabled === "1" ? t("Enabled") : t("Disabled")}
            </Typography>
          </Box>
        </Box>
        <Box>
          <FormControl size="small">
            <Tooltip
              title={t("action.save.changed.associations.tip")}
              placement="top"
            >
              <Select
                sx={{ minWidth: 120 }}
                ...
                onChange={(e) =>
                  handleTrackChange(montageId, screen.id, e.target.value)
                }
                displayEmpty
              >
                <MenuItem value="">
                ...
                {relatedMontage &&
                  Array.from(
                    { length: relatedMontage.tracks_count },
                    (_, i) => (
  ...
  const renderEnvironment = (env) => {
    ...
    const validScreens = env.screens.filter(
    ...
    <Box ref={componentRef}>
      ...
      >
        ...
        >
          {saveError
            ? `Error: ${saveError}`
            : t("action.save.changed.associations")}
        </CustomAlert>
      </CustomSnackbar>
    </Box>
  );
