...
function Playlist({
    ...
    (event) => {
      ...
      if (active.id !== over.id) {
        ...
        console.log("[PlayList] Index search:", {
          ...
          oldIndex,
          newIndex,
          montageIds: playlist.montages.map((m, i) => ({
            index: i,
            id: String(m.id),
            matches: String(m.id) === activeId || String(m.id) === overId,
          })),
        });
  ...
  const update = async (event, name) => {
    ...
    const checks = playlist.montages
      ?.map((montage) => (montage.is_checked === "1" ? "1" : "0"))
      .toString();
  ...
  const handleDoPlayPlaylist = async (playlistId, event) => {
    ...
    if (playlistId === currentPlaylist) {
      console.log(
        `[Playlist handleDoPlayPlaylist] Playlist ${playlistId} is already active.`,
      );
      return;
    }
    ...
    console.log(
      `[Playlist handleDoPlayPlaylist] Switching to playlist: ${playlistId}`,
    );
    ...
    try {
      ...
      console.log(
        `[Playlist] Backend sync completed with result: ${syncSuccess}`,
      );

      if (syncSuccess) {
        console.log(
          `[Playlist handleDoPlayPlaylist] Playlist ${playlistId} is now current.`,
        );
        ...
        >
          ...
          >
            ...
            <Tooltip
              title={
                isCurrent
                  ? t("component.playlist.exhibitions.playing")
                  : t("component.playlist.exhibitions.play")
              }
            >
              <PlaylistPlay
                sx={{
                  ...(isCurrent
                    ? notCurrentPlaylistStyle
...
export default memo(Playlist, (prevProps, nextProps) => {
  const montagesChanged =
    JSON.stringify(prevProps.playlist?.montages) !==
    JSON.stringify(nextProps.playlist?.montages);
...
Playlist.propTypes = {
  ...
  selectedMontagePosition: PropTypes.oneOfType([
    PropTypes.string,
    PropTypes.number,
  ]),
};
