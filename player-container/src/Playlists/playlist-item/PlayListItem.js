 ...
 * ----------------------
 * Renders a single montage item within a playlist with drag-and-drop support.
 *
 ...
 * - playlistId: The actual playlist ID. Used for ALL operations (remove, reorder, etc.).
 *   The removeMontageFromPlaylist callback expects playlistId, NOT playlistIndex.
 */
...
function PlayListItem({
  ...
  const [openPlaylistSelection, setOpenPlaylistSelection] =
  ...
  const handlePlaylistSelectionChange = (event) => {
    ...
    console.log(
      "PlaylistItem: handlePlaylistSelectionChange",
      montage,
      playlistId,
    );
    if (playlistId != -1) {
  ...
  const removeMontage = () => {
    removeMontageFromPlaylist(montageIndex, playlistId);
    console.log(
      "[PlayListItem] removeMontageFromPlaylist:",
      montageIndex,
      playlistId,
    );
  };
  ...
  const handleTitleClick = async () => {
    try {
      ...
      console.log(
        `[MONTAGE_NAVIGATION] Current: ${currentPlaylist}, Target: ${selectedPlaylistId}, MontagePos: ${selectedPlaylistPosition}`,
      );

      if (!isSelectedCurrentPlaylist) {
        // Navigate to different playlist
        console.log(
          `[MONTAGE_NAVIGATION] Loading different playlist: ${currentPlaylist} → ${selectedPlaylistId}`,
        );
        ...
        console.log(
          `[MONTAGE_NAVIGATION] Same playlist, navigating to montage: ${selectedPlaylistPosition}`,
        );
      ...
      >
        ...
        {/* <Checkbox
                    ...
                    onChange={() => {
                        ...
                        handleMontageChecked(montageIndex, playlistId); // Use playlistId, not playlistIndex (see header comment)
                    }}
                /> */}
        <NumberOfTracksButtonContainer className="numberOfTracksButtonContainer">
          <NumberOfTracksButton className="numberOfTracksButton">
            {montage.tracks_count}
          </NumberOfTracksButton>
        ...
        >
          ...
          {isHovered && (
            <Tooltip
              title={t("component.playlist.exhibitions.add-to-playlist")}
            >
              <PlaylistAddIcon
                sx={{ cursor: "pointer" }}
                color="primary"
          ...
          <div>
            <Tooltip
              title={t("component.playlist.exhibitions.associate-displays")}
            >
              <SettingsIcon
                color="primary"
                style={{
          ...
          <Box component="form" sx={{ display: "flex", flexWrap: "wrap" }}>
            <FormControl fullWidth sx={{ m: 1 }}>
              ...
              <Select
                ...
                input={
                  <OutlinedInput label="Playlist" id="playlist-dialog-native" />
                }
              >
                <option key="-1" value="-1">
                  {t("component.playlist.exhibitions.select-playlist-default")}
                </option>
...
PlayListItem.propTypes = {
  ...
  montage: PropTypes.shape({
    ...
    tracks_count: PropTypes.oneOfType([PropTypes.string, PropTypes.number])
      .isRequired,
  }).isRequired,
...
export default React.memo(PlayListItem, (prev, next) => {
  // Find playlists by ID to handle filtered list index mismatch
  const prevPlaylist = prev.playlists.find((p) => p.id === prev.playlistId);
  const nextPlaylist = next.playlists.find((p) => p.id === next.playlistId);
  const prevMontages = prevPlaylist?.montages;
  const nextMontages = nextPlaylist?.montages;
