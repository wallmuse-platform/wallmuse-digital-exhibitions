...
} from "./contexts/EnvironmentsContext.js";
...
import {
  hasStaleAccountFlags,
  cleanupAccountFlags,
} from "./accounts/accountCleanup";
...
function App({ onAppReady }) {
  ...
  useEffect(() => {
    console.log(
      "[App InitMechanism] Initial render complete, calling onAppReady",
    );
...
function DontStartBefore() {
  ...
  console.log(
    "[DontStartBefore DEBUG] isDemoAccount(userId):",
    isDemoAccount(userId),
  );

  // 4. Custom permission hook to use the screen prompty browser one
  const { checkPermission, PermissionDialog } = useScreenPermission({
    t: t,
    onStatusChange: (status) => {
      console.log(
        "[DontStartBefore] Screen permission status changed:",
        status,
      );
    },
  });
  ...
  useEffect(() => {
    if (!isDemo) {
      ...
      console.log(
        "[DontStartBefore] Demo account - auto-completing permission check",
      );
  ...
  console.log(
    "[DontStartBefore] initialLoading: ",
    initialLoading,
    ", syncLoading: ",
    syncLoading,
    ", playlistsLoading: ",
  ...
  if (initialLoading || syncLoading || playlistsLoading) {
    return (
      <div className="loader">
        <p>Loading environment and playlist data...</p>
      </div>
    );
  }
  ...
  if (!house) {
    return (
      <div className="error-message">
        House data is missing. Please try reloading the page.
      </div>
    );
  }
  ...
  if (environments.length === 0) {
    console.log(
      "[DontStartBefore] No environments found, but proceeding - WebPlayer will create default environment",
    );
  }

  if (isDemo && hasStaleAccountFlags()) {
    console.log(
      "[DontStartBefore] Detected stale account flags on demo account, cleaning up",
    );
...
export const handleSendCommand = (command, house) => {
  console.log("[handleSendCommand] DEBUG: Starting command send", {
    command,
    house,
  ...
  if (house === "undefined" || house === "null") {
    console.error(
      '[handleSendCommand] House ID is string "undefined" or "null", cannot send command.',
    );
    return;
  }
...
function PlayerIntegration({ theme, volumeRef, playModeRef }) {
  ...
  const [
    ...
    setSelectedPlaylistPositionForWebPlayer,
  ] = useState(null);
  ...
  console.log(
    "[App PlayerIntegration] isDemo, currentTheme(), theme, userId/session",
    isDemo,
    currentTheme(),
    theme,
    userId,
  );
  const { isMobile, isTablet, isHD, isUHD, isSmartTVHD, isSmartTVUHD } =
    responsiveProps;
  ...
  const calculateOverlayInfo = useCallback(
    (playlistId, montagePosition, track) => {
      ...
      const montageName =
        montage?.name || `Montage ${(montagePosition || 0) + 1}`;
  ...
  const selectedTrackForScreen = useMemo(() => {
    ...
    const activeMontage =
      playlist.montages?.[currentMontageIndex] || playlist.montages?.[0];

    if (!activeMontage) {
      return "1";
    }

    const montageScreenDetail = activeMontage.screens?.find(
      (s) => String(s.id) === String(currentScreenId),
    );
  ...
  const updateCurrentMontageIndex = useCallback((newIndex) => {
    console.log(
      `[App PlayerIntegration Track MONTAGE_INDEX NAV] Updating current montage index from ${currentMontageIndexRef.current} to ${newIndex}`,
    );
  ...
  const getTrackForMontageIndex = useCallback(
    (playlistId, montageIndex) => {
      ...
      const montageScreenDetail = montage.screens?.find(
        (s) => String(s.id) === String(currentScreenId),
      );
  ...
  const useNavigationManager = useCallback(
    (playlistId, montageIndex, track = null) => {
  ...
  const handleMontageNavigation = useCallback(
    (selectedPlaylistId, selectedPlaylistPosition, force = false) => {
      console.log("[NAV] handleMontageNavigation:", {
        selectedPlaylistId,
      ...
      const targetPlaylistObject = playlists.find(
        (p) => String(p.id) === String(selectedPlaylistId),
      );
      if (targetPlaylistObject) {
        window.currentPlaylist = targetPlaylistObject;
        console.log(
          "[NAV] Updated window.currentPlaylist for child iframe access:",
          targetPlaylistObject.name || targetPlaylistObject.id,
        );
      } else {
        console.warn(
          "[NAV] Could not find playlist object for ID:",
          selectedPlaylistId,
        );
      }
      ...
      if (
        ...
        selectedPlaylistPosition === undefined
      ) {
      ...
      if (
        ...
        console.log(
          "[NAV] Skipping duplicate call (use force=true to override)",
        );
        return;
      }
      ...
      const targetPlaylist = isPlaylistChange
        ? selectedPlaylistId
        : currentPlaylist;
      const calculatedTrack = getTrackForMontageIndex(
        targetPlaylist,
        normalizedPosition,
      );
      const overlayInfo = calculateOverlayInfo(
        targetPlaylist,
        normalizedPosition,
        calculatedTrack,
      );
    ...
    [
      playlists,
      getTrackForMontageIndex,
      calculateOverlayInfo,
  ...
  useEffect(() => {
    const handleMontageChange = (event) => {
      ...
      if (montageIndex !== undefined && montageIndex !== null) {
        console.log(
          "[App PlayerIntegration PLAYBACK NAV] Montage changed to index:",
          montageIndex,
        );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "webplayer-playlist-changed",
      handlePlaylistWebSocketChange,
    );
  ...
  useEffect(() => {
    if (playlists && environments) {
      ...
      if (
        !currentPlaylist &&
        !isPlaylistChanging &&
        !window.navigationInitialized
      ) {
        // Use currentPlaylist value (which is undefined for default playlist)
        console.log(
          "[NAV INIT] Using currentPlaylist for initialization:",
          currentPlaylist,
        );
    ...
    currentPlaylist,
    getTrackForMontageIndex,
  ]); // Add currentPlaylist dependency
  ...
  useEffect(() => {
    ...
    if (hasChanged) {
      console.log(
        "[AUTO_TRACK] 🔄 Montage changed to index:",
        currentMontageIndexState,
      );
    ...
    const currentMontageIndex = currentMontageIndexState;
    const currentTrack = getTrackForMontageIndex(
      currentPlaylist,
      currentMontageIndex,
    );
    const overlayInfo = calculateOverlayInfo(
  ...
  console.log(
    "[App PlayerIntegration NAV] Selected track for screen:",
    selectedTrackForScreen,
    "Screen ID:",
    currentScreenId,
  );
  ...
  useEffect(() => {
    ...
    console.log(
      "[App] Sending all track mappings for playlist:",
      currentPlaylist,
      trackMappings,
    );
    ...
    const sendTrackMappings = (retryCount = 0) => {
      if (window.webPlayerSetTrackMappings) {
        console.log(
          "[App] ✅ Calling webPlayerSetTrackMappings with:",
          trackMappings,
        );
        ...
        console.log(
          `[App] ⏳ WebPlayer not ready yet, retrying in 200ms (attempt ${retryCount + 1}/5)`,
        );
        setTimeout(() => sendTrackMappings(retryCount + 1), 200);
      } else {
        console.warn(
          "[App] ⚠️ WebPlayer function not available after 5 retries",
        );
      }
    };
  ...
  const onPause = useCallback(
    () => handleSendCommand('<vlc><cmd action="pause"/></vlc>', house),
    [house],
  );
  ...
  const onVolumeChange = useCallback(
    (v) => {
      ...
      volumeRef.current = v;
      console.log(
        "[PlayerIntegration onVolumeChange] Current volume:",
        volumeRef.current,
      );
      // Send command to house
      handleSendCommand(
        '<vlc><cmd action="volume" param="' + v + '"/></vlc>',
        house,
      );
    },
    [house, volumeRef],
  ); // Only house is a dependency
  ...
  useEffect(() => {
    ...
    // Determine account setup phase - priority ordered for clarity
      ...
      console.log(
        "[App] Setting phase: COMPLETED (activation complete, no refreshes needed)",
      );
      ...
      console.log(
        "[App] Setting phase: OPTIMIZING (activation complete, still refreshing)",
      );
      ...
      console.log(
        "[App] Setting phase: FINALIZING (refreshing environment data)",
      );
    ...
    if (needsRefresh) {
      ...
      setNeedsRefresh(true);
      localStorage.setItem("refreshShown", "true");
      console.log("[App l.526] Updated needsRefresh state:", {
        shouldRefresh: true,
      });
    }
    ...
    if (needsSecondRefresh) {
      console.log(
        "[App] needsSecondRefresh flag detected - performing automatic refresh for screen dimensions",
      );
      ...
      console.log(
        `[App l.536] Incremented refresh attempts to ${newAttemptCount}`,
      );
      ...
      if (newAttemptCount >= 3) {
        ...
        console.log(
          "[App] Executing automatic refresh for screen dimension population",
        );
        setTimeout(() => {
    ...
    return () => {
      window.removeEventListener(
        "account-phase-change",
        handleAccountPhaseChange,
      );
      window.removeEventListener(
        "screen-needs-refresh",
        handleScreenNeedsRefresh,
      );
      window.removeEventListener(
        "activation-complete",
  ...
  const getAccountSetupMessage = () => {
    switch (accountSetupPhase) {
      ...
      case "creating":
        return (
          t("status.account.creating") || "Creating your personal account..."
        );
      case "setup":
        return t("status.account.setup") || "Setting up your environment...";
      case "finalizing":
        return (
          t("status.account.finalizing") ||
          "Finalizing your account setup. Almost ready!"
        );
      case "optimizing":
        return (
          t("status.account.optimizing") ||
          "Optimizing your viewing experience..."
        );
      case "completed":
        return (
          t("success.account.created") ||
          "Your personal account has been created! You can now continue using the application."
        );
      case "retry":
        return (
          t("status.account.retry") ||
          "Account setup needs attention. Please try refreshing the page."
        );
      default:
            ...
            <ThemeProvider theme={theme}>
              ...
              {needsRefresh && !isDemo && (
                <ErrorBoundary
                  fallback={
                    <div>
                      There was an issue with account activation. Please refresh
                      manually.
                    </div>
                  }
                >
              ...
              {accountCreatedSuccess && (
                <CustomSnackbar
                  ...
                  autoHideDuration={
                    accountSetupPhase === "completed"
                      ? 6000
                      : accountSetupPhase === "retry"
                        ? 0
                        : 4000
                  }
                ...
                >
                  <CustomAlert
                    ...
                    severity={
                      accountSetupPhase === "retry" ? "warning" : "success"
                    }
                    onClose={() => setAccountCreatedSuccess(false)}
                  >
